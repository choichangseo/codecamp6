            ...
            <S.Wrapper>
                ...
                {props.data?.fetchBoards.map((el : any, index : any) =>(
                    <S.Row key = {el._id}>
                        ...
                        <S.Title id={el._id} onClick={props.onClickMoveBoardTitle}>{el.title}</S.Title>
                        <S.CreatedDate>{getDate(el.createdAt)}</S.CreatedDate>
                    </S.Row>
                ))}
            </S.Wrapper>
            <S.Footer>
                <S.BoardCreateButton onClick={props.onClickMoveBoarderWrite}>게시물 등록</S.BoardCreateButton>
            </S.Footer>
        </S.ListPageWrapper>
        </>
    )
